...
type FundingSourcePermissionMapping struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.fundingSourceId) || has(self.forProvider.fundingSourceIdRef) || has(self.forProvider.fundingSourceIdSelector) || (has(self.initProvider) && has(self.initProvider.fundingSourceId))",message="spec.forProvider.fundingSourceId is a required parameter"
